...
function StationCard({ station, onClose }: StationCardProps) {
  ...
  const passportHref = new URL(
    `../../constants/passports/${station.Name.toLowerCase()}.txt`,
    import.meta.url,
  ).href;
    ...
    <div className='cards__station'>
      <div className='cards__station-container'>
        ...
        <div className='cards__station__titles'>
          <h3 className='cards__station__title'>
            <strong>{station.Name.toUpperCase()}</strong>
          </h3>
          <a
            href={passportHref}
            className='cards__station__link'
            download={`${station.Name.toLowerCase()}Passport.txt`}
          >
            {passportHref.split('/')[passportHref.split('/').length - 1] === 'undefined'
              ? ''
              : 'паспорт станции'}
          </a>
        </div>
